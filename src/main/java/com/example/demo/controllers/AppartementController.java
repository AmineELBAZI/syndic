...
public class AppartementController {
 ...
 // Get Appartements by Locateur
    ...
    // Delete an Appartement by ID and also delete associated Paiements
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteAppartement(@PathVariable int id) {
        // Find the apartment by ID
        Appartement appartement = appartementRepository.findById(id).orElse(null);
        if (appartement != null) {
            // Delete associated Paiements
            deletePaiementsByAppartement(appartement);
    ...
    // Method to delete all payments associated with an apartment
    private void deletePaiementsByAppartement(Appartement appartement) {
        List<Paiement> paiements = paiementRepository.findByAppartement_Id(appartement.getId());
        paiements.forEach(paiementRepository::delete);
    }
