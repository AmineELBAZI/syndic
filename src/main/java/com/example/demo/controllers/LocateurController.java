...
public class LocateurController {
	...
	/*
	 * @GetMapping("/not-located") public ResponseEntity<List<Locateur>>
	 * getLocateursNotLocated() { List<Locateur> locateursNotLocated =
	 * locateurRepository.findLocateursWithoutAppartement(); return new
	 * ResponseEntity<>(locateursNotLocated, HttpStatus.OK); }
	 */
...
//get locateur by email
    ...
    public ResponseEntity<Object> updateLocateur(@PathVariable int id, @RequestBody Locateur updatedLocateur) {
    	...
    	// Check if an admin with the same email already exists in the database
        Personne existingPerson = personneRepository.findByEmail(updatedLocateur.getEmail());
        if (existingPerson != null) {
            return new ResponseEntity<Object>("Email already exists", HttpStatus.BAD_REQUEST);
        }
        else {
